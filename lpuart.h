...
#define LPUART1 ((struct zmcu_lpuart *)0x40004800)
...
void lpuart_set_baud_rate(struct zmcu_lpuart *lpuart, uint16_t baud_rate);

/* true if the transmission buffer can receive more data */
int lpuart_transmit_ready(struct zmcu_lpuart *lpuart);

/* send a single byte through the transmit data register */
void lpuart_transmit_byte(struct zmcu_lpuart *lpuart, uint8_t byte);

/* true if each byte were transmitted */
int lpuart_transmit_done(struct zmcu_lpuart *lpuart);

/* enable the lpuart peripheral */
void lpuart_enable(struct zmcu_lpuart *lpuart);
